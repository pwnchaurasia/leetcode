...
from lld.library_management.Book import BookLending, Fine, BookReservation
from lld.library_management.constants import AccountStatus, Config, BookStatus, ReservationStatus
...
class Member(Account):
    ...
    def checkout_book_item(self, book_item):
        if self.get_total_books_checked_out() >= Config.MAX_BOOKS_ISSUED_TO_A_USER:
    ...
    def check_for_fine(self, book_item_barcode):
        ...
        if today > due_date:
            diff = today - due_date
            diff_days = diff.days
            Fine.collect_fine(self.get_id(), diff_days)
    ...
    def renew_book_item(self, book_item):
        ...
        if book_reservation is not None and book_reservation.get_member_id() != self.get_id():
            ...
            book_reservation.update_status(ReservationStatus.COMPLETED)
            BookLending.lend_book(book_item.get_bar_code(), self.get_id())
            book_item.update_due_date(
                datetime.datetime.now() + datetime.timedelta(days=Config.MAX_LENDING_DAYS))
            return True
